...
9. **GitHub PRs — CultPodcasts, not the Cursor App.** New cloud agents authenticate as the `cursor` GitHub App: they can **push** `cursor/*` branches but **cannot** create issues/PRs (`must be a collaborator`, `Resource not accessible by integration`). Repo admin is **`cultpodcasts`** (CultPodcastsBot); that is who opens PRs. **Do not stop** at a compare URL. Start `gh` device login, give the operator https://github.com/login/device plus the one-time code, wait until they authorize as **CultPodcasts**, then `gh auth switch --user cultpodcasts` and `gh pr create`. Full steps: **[docs/CLOUD_AGENT_GITHUB_PR.md](docs/CLOUD_AGENT_GITHUB_PR.md)**.
