...
5a. **HARD RULE (14 Sep 2026) — weekly promote: keep prior weeklies published.** Backup → promote new slug → preserve `publishedAt` → GSC notify new post + homepage. **Do not** unpublish the previous weekly after promote. **Do not** add a default EmDash 302 from last Sunday → this Sunday (302 only for true slug moves). Homepage archive ordered by `publishedAt`. Details: **`web/CONTENT_PROMOTION_RUNBOOK.md`** §3 HARD RULE; sibling [freedomtimes-agents/AGENTS.md](../freedomtimes-agents/AGENTS.md) §12.
...
- Do **not** require `EMDASH_TARGET_FINGERPRINT_*` as Cursor secrets for local `deploy-*-local.ps1`. After `web/` build, `Ensure-DeployEmdashTargetFingerprintFromStatus` sets process `EMDASH_TARGET_FINGERPRINT` from `node web/scripts/emdash-core-migrate.mjs print-fingerprint` (same-run `--status --json`). Production local also sets process `EMDASH_TARGET_FINGERPRINT_PRODUCTION` to that value. CI still maps `EMDASH_TARGET_FINGERPRINT_STAGING` / `EMDASH_TARGET_FINGERPRINT_PRODUCTION` → `EMDASH_TARGET_FINGERPRINT`. Do not write the pin into `.env.dev`.
...
| ID | Symptom | Scripted path | If it still fails |
|----|---------|---------------|-------------------|
| CA-01 | Process `TURSO_DATABASE_URL` = production EmDash | Skip process URL (even if Terraform URL is set) and skip process token (`IgnoredProcessProductionShadow`) | Run deploy. **STOP** only if resolve **throws**. Do not write that pair into `.env.dev`. Mint a staging JWT in `.env.dev` |
| CA-06 / CA-07 / CA-23 | No `.terraform` / plugin checksum vs lockfile / output after apply | Auto-init; retry apply once; leave lockfile dirty **through apply and `terraform output`**; revert hash-only after Auth0/worker-name reads | Manual `terraform-run.ps1 -Operation init` then full deploy. Do **not** `git checkout` the lockfile before apply **or** before `terraform output` |
| CA-03 | Missing Terraform Cloud token | Remap `TF_TOKEN_APP_TERRAFORM_IO` → `TF_TOKEN_app_terraform_io` | Copy that Cursor secret into the preflight name in process env (never print it) |
| CA-04 | Wrangler `6111` / `9106` | Replace short `CLOUDFLARE_API_TOKEN` stub with `TF_VAR_CLOUDFLARE_API_TOKEN` | Same overwrite in process env |
| CA-05 | `EBADENGINE` / Node 22.14 | Prepend nvm 22.22.2 | Install that nvm version; prepend its `bin` |
| CA-08 / CA-15 | `EMDASH_TARGET_FINGERPRINT` unset | Local `deploy-*-local.ps1` pins process env from same-run `--status` (`print-fingerprint`) | If apply still refuses, wrong host. Do not invent Cursor secrets |
| CA-14 | Production process URL = production hint; `.env.dev` stays staging | Production resolve uses `TURSO_PRODUCTION_EMDASH_DB_TOKEN` / paired JWT. Never write that pair into `.env.dev` | Copy process JWT into `TURSO_PRODUCTION_EMDASH_DB_TOKEN` only |
| CA-16 | Rollback JSON mtime 1970 / old `createdAtUtc` | Skip check uses `createdAtUtc`; epoch mtime is not fresh | Omit `-SkipTursoBackup`; create a new rollback branch |
| CA-09 | GitHub `Apply to Staging` / Android / iOS fail in ~3s with empty steps | Ignore. Cause is Actions **billing lock** (`job was not started`), not empty YAML | Do not treat as this VM deploy failing. Operator unlocks Actions billing if those PR checks are needed |
| CA-10 | Staging `/` 1–3s with `Server-Timing` | Success | A hang is **0-byte pending**, not a slow 200 |
| CA-11 | `punycode` deprecation / `patch-cloudflare-bundle` patched 0 | Ignore | Noise |
| CA-18 | Agents unset process `TURSO_*` by hand | Scripts skip the process pair; do not unset | Only if resolve still selected production |
| CA-19 | Process `TURSO_DATABASE_URL` equals `TURSO_PRODUCTION_EMDASH_DB_URL` | Run `deploy-*-local.ps1`. Do not abort because process hosts match | STOP only if resolve throws |
| CA-20 | `wsl` / `where.exe` on this Linux VM | Native `turso` (`$HOME/.turso/turso`) and `terraform` on PATH | Interactive `turso auth login` only if platform token missing |
| CA-21 | Terraform `turso_database_auth_token` 404 | Staging: Terraform URL + `.env.dev` token. Production: `TURSO_PRODUCTION_EMDASH_DB_TOKEN` | Mint JWT with `turso db tokens create` after platform CLI login |
| CA-22 | Child `pwsh` missing TF remap / nvm PATH | Do not skip `-LoadEnvFiles` / preflight; use `deploy-*-local.ps1` | Direct `terraform` without `terraform-run.ps1` misses the remap |
| CA-25 | `Missing EMDASH_AUTH_SECRET_PRODUCTION` on secret sync | Skip sync when `.env.production` is missing | Finish `-WorkerOnly -AllowProduction` without `-SyncCloudflareWorkerSecrets` |
...
- Full stack: `pwsh ./scripts/deploy-production-local.ps1` with **no** `-WorkerOnly`. That creates a Turso rollback branch, applies Terraform, skips Worker secret sync when `.env.production` is missing (CA-25), builds, migrates, and deploys. Pass `-SkipTursoBackup` only when the operator **explicitly** orders skip (no 24h-file requirement; `-WorkerOnly` also skips emdash migrate).
...
- Fingerprint: after `web/` build, `Ensure-DeployEmdashTargetFingerprintFromStatus` sets process `EMDASH_TARGET_FINGERPRINT` from `print-fingerprint` (production also sets `EMDASH_TARGET_FINGERPRINT_PRODUCTION`). Do not require those names as Cursor secrets. Do not write the pin into `.env.dev`.
- If Terraform apply **succeeds** and a later step fails: do not re-apply blindly. Finish with `pwsh ./scripts/deploy-production-local.ps1 -WorkerOnly -AllowProduction` (add `-SkipTursoBackup` only when the operator explicitly orders skip). Add `-SyncCloudflareWorkerSecrets` only when `.env.production` exists (CA-25). If Terraform itself failed: **STOP** before wrangler.
