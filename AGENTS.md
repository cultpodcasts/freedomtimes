...
- If Terraform apply **succeeds** and a later step fails: do not re-apply blindly. Finish with `pwsh ./scripts/deploy-production-local.ps1 -WorkerOnly -AllowProduction` (omit `-SkipTursoBackup` unless the skip check passes). Add `-SyncCloudflareWorkerSecrets` only when `.env.production` exists (CA-25). If Terraform itself failed: **STOP** before wrangler.
