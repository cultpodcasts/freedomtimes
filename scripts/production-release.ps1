...
if ($TerraformMode -eq "apply") {
    Write-Host "Reminder: create a Turso rollback branch checkpoint before production apply (runbook §1). CI also backups EmDash Turso, then emdash migrate → wrangler → emdash migrate --check." -ForegroundColor Yellow
