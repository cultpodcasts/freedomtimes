...
variable "page_views_dataset" {
  description = "Workers Analytics Engine dataset name for public page views (also Terraform output page_views_dataset). Must match wrangler.jsonc production analytics_engine_datasets.dataset."
  type        = string
  default     = "freedomtimes_page_views"
}
...
variable "turso_database_group" {
  description = "Turso group for production EmDash database (must match the live database group to avoid replace plans)"
  type        = string
  default     = "freedomtimes-production"
}
