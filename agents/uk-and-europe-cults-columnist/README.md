...
- Optional: `GOOGLE_NEWS_LOCALE_IDS` (comma-separated ids such as `FR-fr,DE-de`) to restrict locales for faster runs. Optional `GOOGLE_NEWS_TOTAL_CAP` (set `> 0`) bounds total RSS discoveries per run; default is unlimited (`0`). Watchlist publishers that share the same localized OR-bundles are merged into one `site:` OR query per chunk (`GOOGLE_NEWS_WATCHLIST_SITE_OR_CHUNK`, default 12) so Google News does more of the ranking work with fewer RSS cells.
