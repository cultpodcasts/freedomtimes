...
| `EMDASH_TARGET_FINGERPRINT` unset on Cloud VM staging | GitHub Action pins are not Cursor secrets here | Local staging `emdash-core-migrate.mjs` uses same-run `emdash migrate --status` fingerprint. Do not invent those secret names. Production-looking hosts still require a pin |
| GitHub `Apply to Staging` / Android / iOS fail in ~3s with empty steps | Workflow dispatch / Capacitor jobs are not this Cloud VM deploy | Ignore unless the operator asked about CI. Local `deploy-staging-local.ps1` is the staging path |
