...
- `tryRefreshAuthCookies()` (`web/src/lib/auth.ts`) is the shared silent-refresh helper. `requireEditorialSession`, `verifyAdminSession` (`/admin/*` pages and `/api/admin/*`), and `/signed-in` call it when `ft_session` is missing or `verifyIdToken()` fails, **or** when `ft_access_token` is missing / its Auth0 API JWT `exp` is within 60s (cookie `maxAge` is 24h; the JWT inside is often shorter). Failed page refresh still clears cookies and redirects to `/auth/login`. `/api/admin/*` now also clears cookies on failed refresh (same as the page path).
...
3. **`web/src/lib/auth.ts`** (`tryRefreshAuthCookies`) is the shared silent-refresh path. Callers invoke it when `ft_session` is missing or fails `verifyIdToken()`, **or** when `accessTokenNeedsRefresh()` is true (missing / unreadable `ft_access_token`, or Auth0 API JWT `exp` within 60s — cookie `maxAge` is 24h; the JWT inside is often shorter). They read `ft_refresh`, call `exchangeRefreshTokenForTokens()` (grant_type `refresh_token` against `/oauth/token`), re-verify the new ID token, run the caller’s role check (`hasEditorialRole` or `hasAdminRole`), and reissue all four auth cookies (Auth0 rotation returns a new `refresh_token` on every use). Used by `editorial-session.ts`, `admin-session.ts` (`/admin`, tips, analytics, push diagnostics), and `/signed-in`. Only if there is no refresh cookie, or the refresh exchange/role-check fails, are cookies cleared (pages redirect to `/auth/login`; `/api/admin/*` also wipes cookies, then 401).
