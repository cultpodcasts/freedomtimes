...
6. Persists **`seo.image`** as the same-origin admin file path **`/_emdash/api/media/file/<storageKey>`** via **`seoImageFieldValueFromNormalizedRow`** (source of truth in this script). EmDash types **`ContentSeo.image`** as **`string | null` only** — objects are rejected by REST validation. A **bare media id** validates but leaves the **admin OG Image** widget empty. Do **not** store a raw **`storageKey`** without the `/_emdash/api/media/file/` prefix (after deduplicated uploads, the R2 key’s ULID can differ from **`media.id`**). Staging example that worked **23 Aug 2026**: `/_emdash/api/media/file/01M0QT8Y0RP9SEZSKVMHXBYEXQ.png`. The public site can still resolve older bare ids in **`resolveSocialImageSrc`**; new writes must use the file path. Writes use **`PUT /_emdash/api/content/posts/:slug`** with a partial body (`seo` only once `social_image` is removed from schema). Agents attaching a pre-generated official card must **not** use MCP **`media_upload` `base64`** — put bytes with **`emdash media upload`** or **`POST /_emdash/api/media`** multipart, then set **`seo.image`** to that file path (see **`docs/CURSOR_EMDASH_MCP.md`** § Large media uploads).
