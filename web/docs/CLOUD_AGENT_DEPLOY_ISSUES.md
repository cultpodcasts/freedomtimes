...
| CA-08 | Staging migrate | `EMDASH_TARGET_FINGERPRINT` unset | GitHub Action pins are not Cursor secrets here | Staging (non-prod URL): `emdash-core-migrate.mjs` uses same-run `--status --json`. **Production-looking URLs require a pin** — local `deploy-*-local.ps1` now pins from same-run `--status` into process `EMDASH_TARGET_FINGERPRINT` (not `.env.dev`, not a Cursor secret) | If apply still refuses, run `node web/scripts/emdash-core-migrate.mjs print-fingerprint` after build against the **intended** host and export `EMDASH_TARGET_FINGERPRINT` |
